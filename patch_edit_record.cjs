...
// Since the new modal also has this string, we should restrict to the edit modal only.
// Let's replace ONLY the one inside editingRecord block.
const startIndex = content.indexOf('editingRecord && (');
if (startIndex !== -1) {
    const endIndex = content.indexOf('</form>', startIndex);
    let block = content.substring(startIndex, endIndex);
    block = block.replace(target, replacement);
    content = content.substring(0, startIndex) + block + content.substring(endIndex);
    fs.writeFileSync('src/App.tsx', content);
} else {
    console.log("Could not find editingRecord block");
}
